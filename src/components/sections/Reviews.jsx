...
const Reviews = () => {
  ...
  const handlePropNameFilterChange = (e) => {
    setPropNameFilter(e.target.value);
    setShowAllReviews(false);
  };
  ...
  const handleWriteClick = () => {
    if (!isLoggedIn) {
      setSnackbarMessage(
        "Please log in to write a review. Don't have an account? Create one."
      );
    ...
    <Container maxWidth="lg">
      ...
      {status === "failed" && (
        <Box
          textAlign="center"
          py={4}
          sx={{ bgcolor: "#ffebee", borderRadius: 2, mb: 3 }}
        >
